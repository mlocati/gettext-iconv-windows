...
function Set-ScriptVar {
    ...
    if ($oldValue -ne '') {
        if ($oldValue -ne $NewValue) {
            throw "'$Name' was previously defined to be '$oldValue', but now we're trying to set it to '$NewValue'."
        }
    } else {
        Set-Variable -Name $Name -Value $NewValue -Scope Script
    }
}
...
foreach ($file in $files) {
    ...
    if ($nameVariableName -ne '') {
        ...
        Set-ScriptVar -Name $nameVariableName -NewValue $versionInfo.ProductName
    }
    if ($versionVariableName -ne '') {
        Write-Output "- $($versionInfo.ProductVersion) => $versionVariableName"
        Set-ScriptVar -Name $versionVariableName -NewValue $versionInfo.ProductVersion
    }
}
